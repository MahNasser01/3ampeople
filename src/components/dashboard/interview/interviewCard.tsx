    ...
    >
      <Card className="relative p-0 mt-4 inline-block cursor-pointer h-60 w-56 ml-1 mr-3 rounded-xl shrink-0 overflow-hidden shadow-md">
        <CardContent className={`p-0 ${isFetching ? "opacity-60" : ""}`}>
          <div className="w-full h-40 overflow-hidden bg-primary-600 flex items-center text-center">
          ...
          <div className="absolute top-2 right-2 flex gap-1">
            <Button
              className="text-xs text-primary-600 px-1 h-6"
              variant={"secondary"}
            ...
            <Button
              className={`text-xs text-primary-600 px-1 h-6  ${
                copied ? "bg-primary-300 text-white" : ""
              }`}
              variant={"secondary"}
              onClick={(event) => {
                event.stopPropagation();
